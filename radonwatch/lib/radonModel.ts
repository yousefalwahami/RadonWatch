...
export function calculateRadonRisk(homeData: HomeData): PredictionResult {
  ...
  factorBreakdown.push({
    ...
    impact: regionMult > 1 ? `+${regionImpact.toFixed(0)} Bq/m³` : `${regionImpact.toFixed(0)} Bq/m³`,
    value: regionImpact,
  });
  ...
  factorBreakdown.push({
    factor: 'Building Age',
    impact: ageMult > 1 ? `+${ageImpact.toFixed(0)} Bq/m³` : `${ageImpact.toFixed(0)} Bq/m³`,
    value: ageImpact,
  });
  ...
  factorBreakdown.push({
    factor: 'Soil Type',
    impact: soilMult > 1 ? `+${soilImpact.toFixed(0)} Bq/m³` : `${soilImpact.toFixed(0)} Bq/m³`,
    value: soilImpact,
  });
  ...
  factorBreakdown.push({
    ...
    impact: sizeMult > 1 ? `+${sizeImpact.toFixed(0)} Bq/m³` : `${sizeImpact.toFixed(0)} Bq/m³`,
    value: sizeImpact,
  });
  ...
  factorBreakdown.push({
    factor: 'Floor Level',
    impact: floorMult > 1 ? `+${floorImpact.toFixed(0)} Bq/m³` : `${floorImpact.toFixed(0)} Bq/m³`,
    value: floorImpact,
  });
  ...
  factorBreakdown.push({
    ...
    impact: ventilationMult > 1 ? `+${ventilationImpact.toFixed(0)} Bq/m³` : `${ventilationImpact.toFixed(0)} Bq/m³`,
...
export const ML_TRAINING_PHASES = [
  ...
  { phase: 'Loading 50,000 Canadian home samples...', progress: 20, duration: 800 },
  { phase: 'Training on geological features...', progress: 40, duration: 1000 },
  { phase: 'Optimizing building characteristics...', progress: 60, duration: 900 },
